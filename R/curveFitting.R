...
#'
curveFit_htppGlobalMah <- function(minObjects, mongoUrl, rerun=FALSE){
  #---------------------------------------------------------------------------------------------#
  # 1. Setup
  #---------------------------------------------------------------------------------------------#
  ...
  if(htpp_global_mah$count() <1 ){
    stop("The htpp_global_mah collection is empty but is required to create htpp_tcpl. Please ensure htpp_global_mah is created before proceeding.")
  }
...
#'
curveFit_htppCatMah <- function(minObjects, mongoUrl, rerun=FALSE, nThreads=1){
      ...
      for(Category in unique(DistanceData$category_name_r)){
        ...
        concResp <- foreach(x=Chems, .combine = 'rbind', .packages = c('tidyr', 'dplyr', 'tcplfit2')) %dopar% {
          ...
          for(sample_type in unique(DistanceData_1Category$stype[DistanceData_1Category$chem_id == x & DistanceData_1Category$stype != "vehicle control"])){
            ...
            # if(length(htpp_tcpl$find(query = mongoQuery(pg_id = PG, cell_type = cell, chem_id = Chem, stype = sample_type, approach = "category", endpoint = Category), fields = '{"_id" : 1}')) > 0){
            #
            #   cat(PG, " ", cell, " ", Chem, " ", Category, " old results deleted!\n")
            #
            #   htpp_tcpl$remove(query = mongoQuery(pg_id = PG, cell_type = cell, chem_id = Chem, stype = sample_type, approach = "category", endpoint = Category))
            # }
...
#'
curveFit_htppFeature <- function(minObjects, mongoUrl, rerun=FALSE, nThreads=1){
  #---------------------------------------------------------------------------------------------#
  ...
  FeatureList <- FeatureList %>% select(any_of(c("feature_id", "feature_name_mongo", "feature_name_r", "category_name_r"))) %>% filter(!is.na(feature_id))
  ...
  i <- 1
  for(PG in PlateGroupList){
    featureCount[i] <- length(htpp_tcpl$distinct(key = "chem_id", query = mongoQuery(stype = c("test sample", "null", "reference chemical", "viability positive control"), pg_id = PG, approach = "feature")))
    i <- i+1
  }
