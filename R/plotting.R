...
#'
cellViability_plots <-function(file_path, study_name, mongoUrl, refChems = TRUE){

  # 1. Generate cv curve plots

  cv_tcpl <- data.table(mongo(collection="cv_tcpl", url=mongoUrl, verbose=getOption("verbose"))$find())
  if (length(cv_tcpl)<1){
...
#'
viability_controlPlot_htppWell<-function(file_path, vehicle_control, viability_positive_control, study_name, mongoUrl){
...
viability_controlPlot_htppWellNorm <- function(mongoUrl, file_path, vehicle_control, viability_positive_control, study_name){
  ...
  for(cell in unique(CCData[, cell_type])){
    dat <- CCData[cell_type == cell]
    ggplot(dat, aes(x=as.numeric(pg_id), y=rel_cell_count, color=chem_id, shape=chem_id)) +
      geom_jitter(width=0.2, height=0, alpha=0.5) +
      scale_color_manual(values=c("gray50", "red")) +
      theme_bw() + ylim(0, NA) + scale_x_continuous(breaks=seq(0,30, by=2)) +
      ggtitle(paste(study_name, cell, sep = " "), subtitle=paste0("htpp_well: relative cell count\n", Sys.Date()))
...
#'
curvePlots_htppGlobalMah <- function(file_path, study_name, mongoUrl){
  ...
  for(cell in htpp_well_trt$distinct(key = "cell_type")){
    ...
    RefChem <- Subset %>% filter(stype %in% c("reference chemical") & chem_id != "TSA") %>% dplyr::mutate(pg_id = as.numeric(pg_id))
    ...
    NullChem <- Subset %>% filter(stype %in% c("null")) %>% dplyr::mutate(pg_id = as.numeric(pg_id)) %>%
...
#'
curvePlots_htppCatMah <- function(file_path, study_name, mongoUrl){
  ...
  for(cell in htpp_well_trt$distinct(key = "cell_type")){
    ...
    RefChem <- CPData %>% filter(stype %in% c("reference chemical")) %>% dplyr::mutate(pg_id = as.numeric(pg_id)) %>%
      filter(hitcall  >0.5) %>%
    ...
    NullChem <- CPData %>% filter(stype %in% c("null")) %>% dplyr::mutate(pg_id = as.numeric(pg_id)) %>%
    ...
    ZF_Null <- CPData %>% filter(stype %in% c("null")) %>% dplyr::mutate(pg_id = as.numeric(pg_id)) %>%
    ...
    ZF <- CPData %>% dplyr::mutate(pg_id = as.numeric(pg_id)) %>%
...
#'
curvePlots_htppFeature <- function(file_path, study_name, mongoUrl) {
  ...
  for(cell in htpp_well_trt$distinct(key = "cell_type")){
    ...
    RefChem <- CPData %>% filter(stype %in% c("reference chemical") & chem_id != "TSA") %>%
      ...
      dplyr::mutate(pg_id = as.numeric(pg_id),
    ...
    for(Chem in chemList){
      ...
      ggplot(Subset, aes(x = BMC, y = EffectSize)) +
        ...
        scale_x_log10(breaks = c(10^c(-6:2)), labels = c(-6:2)) +
...
pseudoBmcPlots_htppCatMah <- function(file_path, study_name, mongoUrl, bmc_min=10^0.5){
  ...
  for(cell in unique(CPData[, cell_type])){
    ...
    for(Chem in unique(RefChem$chem_id)){
      ...
      ggplot(Subset, aes(x = bmc, y = top_over_cutoff)) +
        facet_wrap(~as.numeric(pg_id), nrow = 4) +
        geom_point(color = ColumnColors(Subset$endpoint, "vector"), shape = PchShape(Subset$endpoint), size = 3, alpha = 0.7) +
        geom_text(label = GOLetter(Subset$endpoint), size = 3) +
        geom_rect(data = DoseRange, aes(xmin = min_conc, xmax = max_conc, ymin = 0, ymax = 1), fill = "gray50", color = "transparent", alpha = 0.3) +
        scale_x_log10(breaks = c(10^c(-6:2)), labels = c(-6:2)) +
...
#'
pacPlots_htppGlobalMah <- function(file_path, study_name, mongoUrl){
  ...
  ggplot(Data, aes(x = pac, y = stype)) +
    ...
    scale_color_manual(values = c("cornflowerblue", "chocolate3", "darkgoldenrod1", "gray30", "red", "blue")) +
...
#'
pacPlots_htppCatMah <- function(file_path, study_name, mongoUrl){
  htpp_pac<-mongo(collection = "htpp_pac", url = mongoUrl, verbose = getOption("verbose"))
